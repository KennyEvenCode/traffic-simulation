...
road.prototype.initRegularVehicles = function (densityPerLane, fracTruck) {
  ...
  for (var i = 0; i < nvehPlus; i++) {
    ...
    var vehicle_type = Math.random() < fracTruck ? "truck" : "car"; //TODO add semitruck
    var vehicle_length = vehicle_type === "car" ? car_length : truck_length; //TODO carlength
    //var vehicle_length= (vehicle_type === "car") ? car_length : (vehicle_type === "truck") ? truck_length : semitruck_length; //TODO carlength
    var vehicle_width = vehicle_type === "car" ? car_width : truck_width; //TODO carwidth
    //var vehicle_width=(vehicle_type === "car") ? car_width : (vehicle_type === "truck") ? truck_width : semitruck_width; //TODO carwidth
    ...
    var sFollow = followerInfo[0] ? u - vehicle_length - follower.u : 1000;
...
road.prototype.writeVehicleLongModels = function (umin, umax) {
  console.log(
    "\nin road.writeVehicleLongModels(): ID=",
    this.roadID,
    " nveh=",
    this.veh.length,
    " itime=" + itime
    ...
    if (this.veh[i].u >= uminLoc && this.veh[i].u <= umaxLoc) {
      console.log(
        " veh[" +
          ...
          "  T=" +
          parseFloat(this.veh[i].longModel.T).toFixed(1) +
          "  s0=" +
          parseFloat(this.veh[i].longModel.s0).toFixed(3) +
          "  a=" +
          parseFloat(this.veh[i].longModel.a).toFixed(3) +
          "  b=" +
          parseFloat(this.veh[i].longModel.b).toFixed(3) +
          "  noiseAcc=" +
...
road.prototype.writeVehicleLCModels = function () {
  console.log(
    "\nin road.writeVehicleLCModels(): ID=",
    this.roadID,
    " nveh=",
    this.veh.length,
    " itime=" + itime
...
road.prototype.testCRG = function (xUser, yUser) {
  ...
  for (var i = 0; i <= this.nSegm; i++) {
    var dist2 =
      Math.pow(xUser - this.xtab[i], 2) + Math.pow(yUser - this.ytab[i], 2);
    if (dist2 < dist2_min) {
      dist2_min = dist2;
      this.iPivot = i;
      this.xPivot = xUser;
  ...
  if (success) {
    console.log(
      "road.testCRG: new CRG event initiated!",
      " dist_min=",
      Math.round(dist_min),
      " iPivot=",
      this.iPivot,
      " xPivot=",
      Math.round(this.xPivot),
      " yPivot=",
      Math.round(this.yPivot)
    );
  }
  return [
    success,
      ...
      if (u > uBegin - inflLen && u < uBegin + commonLen + inflLen) {
        ...
        var approachFact =
          u < uBegin
            ...
            : u > uBegin + commonLen
            ? Math.pow(Math.sin(0.5 * Math.PI * urelEnd), 2)
            : 0;
        ...
        deltaX[i] =
          approachFact * deltaXold + (1 - approachFact) * deltaR_dot_e * e[0];
        deltaY[i] =
          approachFact * deltaYold + (1 - approachFact) * deltaR_dot_e * e[1];
      }
    }
  }
...
road.prototype.finishCRG = function () {
  ...
  for (var inew = 1; inew < this.nSegm; inew++) {
    ...
    xtabNew[inew] =
      (1 - rest) * this.xtab[iUpper - 1] + rest * this.xtab[iUpper];
    ytabNew[inew] =
      (1 - rest) * this.ytab[iUpper - 1] + rest * this.ytab[iUpper];

    if (false) {
      var drnew = Math.sqrt(
        Math.pow(xtabNew[inew] - xtabNew[inew - 1], 2) +
  ...
  if (false) {
    ...
    for (var i = 0; i <= this.nSegm; i++) {
      console.log(
        ...
        i,
        " xytabOld=",
        Math.round(this.xtabOld[i]),
        "",
        Math.round(this.ytabOld[i]),
        " xytab=",
        Math.round(this.xtab[i]),
        "",
        Math.round(this.ytab[i]),
        " xytabNew=",
        Math.round(xtabNew[i]),
        "",
        Math.round(ytabNew[i]),
        " du=",
        parseFloat(chdUPoints[i] - chdUPoints[Math.max(i - 1, 0)]).toFixed(2),
        " duNew=",
        parseFloat(this.roadLen / this.nSegm).toFixed(2)
      );
    }
  }
...
road.prototype.smoothLocally = function (iCenter, iWidth) {
  ...
  for (var i = iCenter - iw; i <= iCenter + iw; i++) {
    if (isNaN(xtabNew[i]) || isNaN(ytabNew[i])) {
      console.log(
        "road.SmoothLocally: i=",
        i,
        " iCenter=",
        iCenter,
        "iw=",
        iw,
        ": xtabNew[i] or ytabNew[i] are NaN's!" + "\ndoing nothing"
      );
      return;
    }
  }
...
road.prototype.updateTruckFrac = function (fracTruck, mismatchTolerated) {
  if (this.veh.length > 0) {
    ...
    if (Math.abs(fracTruckReal - fracTruck) > mismatchTolerated) {
      var addTruck = nTruckDesired > nTruck;
      var newType = !addTruck ? "car" : "truck";
      //var newType = !addTruck ? "car" : (Math.round(Math.random()) == 1) ? "truck" : "semitruck"; //TODO
      var newLength = addTruck ? truck_length : car_length; //TODO carlength
      //var newLength= (newType === "car") ? car_length : (newType === "truck") ? truck_length : semitruck_length; //TODO
      var newWidth = addTruck ? truck_width : car_width; //TODO carwidth
      //var newWidth=(newType === "car") ? car_width : (newType === "truck") ? truck_width : semitruck_width; //TODO

      var newLongModel = addTruck ? longModelTruck : longModelCar;
      var diffSpace = (addTruck ? -1 : 1) * (truck_length - car_length);
      //var diffSpace = (addTruck ? -1 : 1) * (truck_length - car_length); //TODO carlength
      var success = 0; // false at beginning

      // find the candidate vehicle (truck or car) with the largest lag gap

      var candidateType = addTruck ? "car" : "truck";
    ...
    if (beyondStart && outsideLCban && this.veh[i].isRegularVeh()) {
      ...
      if (targetLaneExists && lastChangeSufficTimeAgo) {
        ...
        if (
          ...
          var accLagNew = this.veh[iLagNew].longModel.calcAcc(
            sLagNew,
            speedLagNew,
            speed,
            accNew
...
*/
      ...
      if (connecting) {
        ...
        if (!laneContinues) {
          //!! need forbidding changing back to the
          // closing lane?
        ...
        if (laneContinues) {
            ...
            if (followerExists) {
              ...
              if (this.connectLog) {
                console.log(
                  ...
                  targetRoad.veh[iFollow].id,
                  "\n       tc=expected dt @u Source->uTarget=",
                  tc.toFixed(1),
          ...
          if (this.connectLog) {
            console.log(
              "  Preparation (ii): Potential acceleration if",
          ...
          if (leaderExists) {
            ...
            accGo = Math.min(
              accDefault,
              vehConnect.longModel.calcAcc(sLead, speed, speedLead, accLead)
            ); // !! influence
            if (this.connectLog) {
              console.log(
                "  Preparation (iiia): leaderExists: leader id=",
                targetRoad.veh[iLead].id,
                " s=",
                s,
                " accGo=",
            ...
            if (potentialConflictsExist) {
              ...
              if (this.connectLog) {
                console.log(
                  "  Preparation (iiib): leader and",
                  "potential conflicts exist:",
                  "targetCanBeEntered=",
                  targetCanBeEntered,
                  "s to target leader s=",
                  s.toFixed(1),
                  "speedLead=",
                  speedLead.toFixed(1),
                  "accLead=",
          ...
          if (followerExists) {
            if (C2 || C3) {
              // then always braking in region C1
              ...
              if (this.connectLog) {
                console.log(
                  ...
                  "targetRoad.veh[iFollow].longModel.cool=",
                  targetRoad.veh[iFollow].longModel.cool,
                  "sFollow=",
                  sFollow.toFixed(1),
                  "speedFollow=",
                  speedFollow.toFixed(1),
                  "speed=",
                  vehConnect.speed.toFixed(1),
        ...
        if (u > uSource - distBeforeEnd && this.roadID != targetRoad.roadID) {
          ...
          if (this.connectLog) {
            ...
            console.log(
              ...
              targetRoad.roadID,
              "at u=",
              transferredVeh.u.toFixed(1),
              "lane=",
              transferredVeh.lane,
              "laneOld=",
              transferredVeh.laneOld,
              "v=",
              transferredVeh.v.toFixed(1),
              "\ntransferredVeh=",
              transferredVeh,
              "\n========================================================="
            );
...
road.prototype.determineConflicts = function (
      ...
      if (isCandidate) {
        ...
        if (log) {
          console.log(
            ...
            vehConflict.id,
            " uOther=",
            vehConflict.u.toFixed(1),
            " ucOther=",
            ucOther.toFixed(1),
            " speedOther*tc=",
            (vehConflict.speed * tc).toFixed(1),
            " ducOther_after_tc=",
            ducOtherVeh.toFixed(1),
            "\n                                          xtc=",
            xtc.toFixed(1),
            " ttc=",
            ttc.toFixed(1),
        ...
        if (log) {
          ...
          if (dist2 >= 16) {
            console.log(
              ...
              "yConflictOwn=",
              yConflictOwn.toFixed(1),
              "\n    xConflictOther                  =",
              xConflictOther.toFixed(1),
              "yConflictOther=",
              yConflictOther.toFixed(1),
              "duOwn(now)=",
              duOwn.toFixed(1),
              "ducOtherVeh(now)=",
  ...
  if (log) {
    console.log(
      "Leaving determineConflicts, conflictsExist=",
      conflictsExist,
      "\n=============================================\n\n"
    );
  }
...
road.prototype.mergeDiverge = function (
  ...
  var paddingLTC = isMerge && prioOwn ? this.paddingLTC : 0; // visib. extension for target drivers to orig vehs
  ...
  if (log) {
    console.log(
      "\n\nin road.mergeDiverge: itime=",
      itime,
      " ID=",
      this.roadID,
      " targetVehicles.length=",
      targetVehicles.length,
      " originVehicles.length=",
      originVehicles.length
    );
  }
  ...
  if (isMerge && loc_prioOwn) {
    ...
    for (var j = 0; j < targetVehicles.length; j++) {
      ...
      var accTargetStop = targetVehicles[j].longModel.calcAcc(
        sStop,
        speedTarget,
        0,
        0
      ...
      if (iLast > -1 && targetVehicles[j].isRegularVeh()) {
        ...
        if (lastOrigIsLeader) {
          ...
          var accLTC = targetVehicles[j].longModel.calcAcc(
            s,
            speedTarget,
            speedOrig,
            0
        ...
        if (false) {
          console.log(
            ...
            targetVehicles[j].id,
            " iLast id=",
            originVehicles[iLast].id,
            " lastOrigIsLeader=",
            lastOrigIsLeader,
            " sStop=",
            parseFloat(sStop).toFixed(1),
            " accTargetStop=",
            parseFloat(accTargetStop).toFixed(1),
            " acc=",
            parseFloat(targetVehicles[j].acc).toFixed(1)
          );
        }
      }
    }
...
road.prototype.updateDensity = function (density) {
  ...
  // too few vehicles, generate one per time step in largest gap
    ...
    var vehicle_type = Math.random() < fracTruck ? "truck" : "car";
    var vehicle_length = vehicle_type === "car" ? car_length : truck_length; //TODO carlength
    var vehicle_width = vehicle_type === "car" ? car_width : truck_width; //TODO carwidth
    ...
    if (!emptyLanes) {
      ...
      success = maxSpace > car_length + 2 * this.veh[k].longModel.s0; //TODO carlength
    }
    ...
    if (success) {
      ...
      if (!emptyLanes) {
        uNew = this.veh[k].u + 0.5 * (car_length + maxSpace); //TODO carlength
...
road.prototype.updateBCup = function (Qin, dt, route) {
  ...
  if (this.inVehBuffer >= this.randomValBCup) {
    ...
    // if((!success) && setTrucksAlwaysRight && (vehicle_type==="truck"))
    ...
    if (!success && (!this.setTrucksAlwaysRight || vehicle_type == "car")) {
      var spaceMax = 0;
    ...
    if (success) {
      var longModelNew = vehicle_type === "car" ? longModelCar : longModelTruck;
      var LCModelNew = vehicle_type === "car" ? LCModelCar : LCModelTruck;
      var uNew = 0;
      ...
      //if((lane!=this.nLanes-1)&&(vehicle_type==="truck")){
      if (false) {
        console.log(
          "road.updateBCup: ID=",
          this.roadID,
          " new vehicle at pos u=0, lane=",
          lane,
          " type=",
          vehicle_type,
          " s=",
...
road.prototype.updateModelsOfAllVehicles = function (
  ...
  if (deepCopying) {
    for (var i = 0; i < this.veh.length; i++) {
      ...
      if (false) {
        console.log(
          "updateModelsOfAllVehicles: type=",
          this.veh[i].type,
          " speedl=",
          this.veh[i].longModel.speedlimit,
        ...
        if (nextOfframpNearby) {
          ...
          if (tacticalLC) {
            ...
            if (deepCopying) {
              ...
              thisVeh.longModel =
                thisVeh.type === "truck"
                  ? this.longModelTacticalTruck
                  : this.longModelTacticalCar;
            }
  ...
  for (var i = 0; i < this.veh.length; i++) {
    ...
    if (filterPassed) {
      ...
      if (false) {
        ...
        console.log(
          "drawVehicle after getTraj: roadID=",
          this.roadID,
          " time=",
          time,
          " vehid=",
...
road.prototype.drawVehicle = function (
  ...
  var vehicle_pixel_length = vehSizeShrinkFactor * scale * this.veh[i].len;
  var vehicle_pixel_width = scale * this.veh[i].width;
  ...
  vehicle_image =
    type === "car" ? carImg : type === "truck" ? truckImg : obstacleImg;
  ctx.setTransform(
    cphiVeh,
    -sphiVeh,
    +sphiVeh,
  ...
  ctx.drawImage(
    vehicle_image,
    -0.5 * vehicle_pixel_length,
    -0.5 * vehicle_pixel_width,
    vehicle_pixel_length,
    vehicle_pixel_width
  );
  ...
  if (type != "obstacle" && speedmax > 1e-10) {
    ...
    var effLenPix =
      type === "car" ? 0.95 * vehicle_pixel_length : 0.9 * vehicle_pixel_length;
    var effWPix =
      type === "car" ? 0.55 * vehicle_pixel_width : 0.7 * vehicle_pixel_width;
    var speed = this.veh[i].speed;
...
road.prototype.updateSpeedlimits = function (trafficObjects) {
  ...
  for (var i = 0; i < trafficObjects.trafficObj.length; i++) {
    ...
    if (
      ...
      if (false) {
        console.log(
          "road.updateSpeedlimits: speed limit ",
          formd(speedL),
          " starting at ",
          formd(obj.u)
        );
  ...
  if (false) {
    for (var iveh = 0; iveh < this.veh.length; iveh++) {
      ...
      if (veh.isRegularVeh()) {
        console.log(
          "end updateSpeedlimits: u=",
          veh.u,
          "speedlimit=",
          veh.longModel.speedlimit
        );
      }
    }
  }
...
road.prototype.dropObject = function (trafficObj) {
  ...
  console.log(
    "itime=",
    itime,
    " in road.dropObject: trafficObj.u=",
    u,
    " trafficObj.lane=",
    lane,
    " this.nLanes=",
  ...
  if (trafficObj.type === "obstacle") {
    ...
    console.log(
      "  end road.dropObject: dropped obstacle at uDrop=",
      u,
      " lane=",
      lane,
      " id=",
